    ...
    if ongoing_attacks:
        ...
        if user_id in admin_owner:
            ...
            if len(command) == 4:  # Ensure proper command format (no threads argument)
                ...
                response = "Usage: /one <target> <port> <time>"
        else:
            ...
            if user_id in bgmi_cooldown:
                ...
                if current_time < cooldown_expiration:
                    ...
                    response = f"You need to wait {time_left} seconds before using the /one command again."
                else:
                    ...
                    if len(command) == 4:  # Ensure proper command format (no threads argument)
                        try:
                            ...
                            if time > 180:
                                ...
                                start_attack_reply(message, target, port, time)
                                bgmi_cooldown[user_id] = datetime.now(pytz.timezone('Asia/Kolkata')) + timedelta(minutes=2)
                                return  # Early return since response is handled in start_attack_reply
                        except ValueError:
                            response = "Error: Please ensure port and time are integers."
                    else:
...
def show_help(message):
    ...
    - /one : Execute a BGMI server attack (specific conditions apply).
...
def welcome_plan(message):
    ...
    Purchase VIP DDOS Plan From @OneXDeba
    Join Channel @kuch nhi hai
'''
    bot.reply_to(message, response)
